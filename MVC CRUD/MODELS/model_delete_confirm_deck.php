...
$table='decks';  //table to delete values from
$msg='';  //this is an empty message initially
//$_SESSION['validID']=FALSE; //reset to FALSE 
	
//get the values entered in the form
$deckID=$conn->real_escape_string($_POST['deckID']);
$_SESSION['deck_id']=$deckID;

//Construct Query strings
$sqlData= "SELECT * FROM $table WHERE deck_id='$deckID'";
$sqlTitles = "SELECT column_name as Field
from `information_schema`.`columns` 
where `table_schema` = 'k00223356_duelnetwork' and `table_name` in ('decks')
and `column_name` in ('deck_name')";  //get the table column descriptions
